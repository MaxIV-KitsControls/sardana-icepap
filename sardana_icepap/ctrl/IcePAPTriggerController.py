...
class IcePAPTriggerController(TriggerGateController):
    ...
    ctrl_properties = {
        ...
        "IcepapCtrlAlias": {Type: str, Description: "The host name"},
        "AxisInfos": {
            Type: str,
    ...
    def __init__(self, inst, props, *args, **kwargs):
        ...
        self._ipap = icepap.IcePAPController(
    ...
    def _set_out(self, out=LOW, axis=0):
        ...
        if axis == 0:
            motor.syncaux = value
            if self._ecam_source_dict and self._motor_axis in self._ecam_source_dict:
        ...
        if axis == 0:
            ...
            for p in pmux:
                if "E0" in p:
                    ...
                    break
            self._ipap.add_pmux(self._motor_axis, "e0", pos=False, aux=True, hard=True)
        ...
        if axis == 0:
            for i in self._ipap.find_axes():
                ...
                try:
                    ...
                    self._log.error(
                        "Axis %s not used by Sardana (no " "alias)", motor_name
                    )
                    pass
    ...
    def StartOne(self, axis):
        ...
        if self._is_tgtenc:
            self._log.info("Send ESYNC to motor: %s", self._ipap[self._motor_axis].name)
    ...
    def SynchOne(self, axis, configuration):
        ...
        if SynchParam.Initial not in synch_group:
            ...
            if nr_points > 1:
                msg = (
                    "The IcePAP Trigger Controller is not allowed to "
        ...
        try:
            tgtenc_cfg = self._ipap[self._motor_axis].get_cfg("TGTENC")["TGTENC"]
        ...
        if enc != "AXIS":
            ...
            enc_resol = int(motor.get_cfg(cfgstep)[cfgstep]) / int(
                motor.get_cfg(cfgturn)[cfgturn]
            )
            motor_resol = int(motor.get_cfg("ANSTEP")["ANSTEP"]) / int(
                motor.get_cfg("ANTURN")["ANTURN"]
            )
        ...
        if self._start_trigger_only:
            ...
            msg = (
                "The Trigger by position not accept more than {0} "
                "positions (points)".format(MAX_ECAM_VALUES)
            )
            raise RuntimeError(msg)
        else:
            trigger_table = numpy.linspace(start, end - delta, int(nr_points))
            self._log.debug(
                "Table generated by numpy.linspace({0},{1},"
        ...
        for i in range(self._retries_nr):
            try:
                ...
                self._log.warning("Send trigger table error retry: " "{0}".format(i))
        if not table_loaded:
    ...
    def SetAxisPar(self, axis, par, value):
        if axis == 0:
            ...
            raise ValueError("unsupported axis par {} for axis {}".format(par, axis))
