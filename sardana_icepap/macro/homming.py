...
def home(macro, motorsInfoList, group=False, strict=False):
    ...
    (
        ...
        HM_ENCIN_CMD,
        ABORT_CMD,
    ) = populate_homing_commands(motors, hmDirections, group=group, strict=strict)
    ...
    try:
        while True:
            ...
            for i, motInfo in enumerate(motorsInfoList):
                ...
                macro.debug(
                    "Motor: %s, position: %s", motor.alias(), motInfo["position"]
                )
                ...
                if homingStatus == "FOUND":
                    motInfo["homed"] = True
                    axis = motor.getAxis()
                    ans = pool.SendToController([ctrlName, HM_POS_CMD.format(axis)])
                    motInfo["home_ice_pos"] = ans
                    ans = pool.SendToController([ctrlName, HM_ENCIN_CMD.format(axis)])
                    motInfo["encin"] = ans
...
class ipap_homing(Macro):
    ...
    param_def = [
        ...
        [
            ...
            None,
            "List of motors and homing directions.",
        ],
    ]

    result_def = [["homed", Type.Boolean, None, "Motors homed state"]]
    ...
    def prepare(self, *args, **opts):
        ...
        self.motorsInfoList = [
            create_motor_info_dict(m, d) for m, d in motors_directions
        ]

        # getting motion object for automatic aborting
        motorNames = [
            motorInfoDict["motor"].name for motorInfoDict in self.motorsInfoList
        ]
